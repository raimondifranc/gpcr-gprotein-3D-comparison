...
for ii in range(len(Gpcr_r.keys())):
  for jj in range(ii+1, len(Gpcr_r.keys())):
    ...
    print ("./bio_align_fit_n_rmsd.py -refe %s -mobi %s -r_fit_chain %s -m_fit_chain %s -r_fit_sele %s -m_fit_sele %s -r_rms_chain %s -m_rms_chain %s -r_rms_sele %s -m_rms_sele %s > %s_%s_fit.txt" % (pdb1, pdb2, rc1, rc2, r_res1, r_res2, mc1, mc2, m_res1, m_res2, pdb1.split("/")[1].split(".cif")[0], pdb2.split("/")[1].split(".cif")[0]))
